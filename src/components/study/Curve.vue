...
export default {
  ...
  methods:{
    draw() {
      ...
      let length = Math.sqrt(Math.pow(target.y-source.y,2)+Math.pow(target.x-source.x,2)) //向量长
      ...
      path
        .attr('d',function(d){
            return "M 0 0 Q"+(middle-delta)/2+' '+(offset/2+offset/4)+' '+(middle-delta)+' '+offset
                 + "M"+(middle+delta)+' '+offset+" Q"+(middle+delta+length)/2+' '+(offset/2+offset/4)+' '+length+' 0';  // 结尾不能加Z，否则箭头绘制出错
        })
      text
        .attr('x',middle)
        .attr('y',offset)
        .attr('transform', 'rotate(180 ' + middle + ' ' + offset + ')');
